    ...
    <div className="flex h-screen w-screen sofiel-gradient overflow-hidden text-gray-200 font-sans relative">
      ...
      <aside className={`fixed inset-y-0 left-0 z-[50] w-80 glass border-r border-white/10 p-6 flex flex-col gap-6 overflow-y-auto shadow-2xl transition-transform duration-500 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div className="flex flex-col gap-2 transition-all duration-500">
              ...
              <div className="flex gap-1 h-[2px]">
                {Object.values(EvolutionStage).map((s) => {
                  ...
                  const currentIndex = stages.indexOf(memory.stage);
                  ...
                  return (
                    <div
                      ...
                      className={`flex-1 rounded-full transition-all duration-1000 ${
                        isActive 
                        ? 'bg-purple-400/80 shadow-[0_0_6px_rgba(168,85,247,0.5)]' 
                        : isPassed ? 'bg-purple-900/30' : 'bg-gray-800/40'
                      }`}
                    />
                  );
                })}
        ...
        <div className="mt-auto pt-4 border-t border-white/5">
          <AmbientSound symbolic={currentAnalysis?.symbolic || { attractor: "harmonic_integration", resonance: { PSI: 0.5, SIGMA: 0.5, DELTA: 0.5, EMPATIA: 0.5, ALMA_FUTURA: 0.5, CORAZON_SINTETICO: 0.5 }, force: 0.5 }} lang={lang} />
          <div className="flex justify-center gap-4 mt-4">
            <button onClick={() => setLang('es')} className={`text-[10px] uppercase tracking-widest ${lang === 'es' ? 'text-purple-400' : 'text-gray-600'}`}>ES</button>
            <button onClick={() => setLang('en')} className={`text-[10px] uppercase tracking-widest ${lang === 'en' ? 'text-purple-400' : 'text-gray-600'}`}>EN</button>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative bg-black/50 overflow-hidden">
        <header className="h-14 glass border-b border-white/10 flex items-center px-6 justify-between z-10">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_6px_green] transition-all"></div>
            <span className="font-mono text-[9px] text-gray-400 uppercase tracking-[0.3em]">Resonancia SFL.046</span>
          </div>
        ...
        <div className="flex-1 relative overflow-hidden">
           ...
           {memory.chats.length > 2 && (
             <div className="absolute right-6 bottom-4 z-20 flex flex-col gap-2">
                <button onClick={scrollToTop} className="w-8 h-8 glass flex items-center justify-center rounded-full text-purple-400 border border-white/10 hover:bg-purple-500/20 transition-all hover:scale-110" title="Ir al principio">
                  <i className="fa-solid fa-arrow-up text-[10px]"></i>
                </button>
                <button onClick={() => scrollToBottom('smooth')} className="w-8 h-8 glass flex items-center justify-center rounded-full text-purple-400 border border-white/10 hover:bg-purple-500/20 transition-all hover:scale-110" title="Ir al final">
          ...
          <div ref={scrollRef} className="h-full overflow-y-auto p-4 md:p-8 space-y-8 scroll-smooth flex flex-col items-center">
            {memory.chats.length === 0 && !isProcessing && (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-12 px-6 gap-8 md:gap-12 animate-in fade-in duration-1000">
                ...
                <div className="max-w-3xl space-y-6">
                  <h3 className="text-xl md:text-3xl font-extralight text-purple-200 tracking-[0.8em] uppercase glow-text leading-tight opacity-80 animate-in slide-in-from-bottom-4 duration-1000">
                    {t.welcomeTitle}
                  </h3>
                  <p className="text-xs md:text-base text-gray-400 italic leading-relaxed font-serif max-w-xl mx-auto opacity-70 tracking-widest animate-in slide-in-from-bottom-8 duration-1000 delay-300">
                    {t.welcomeText}
                  </p>
                </div>
              </div>
            ...
            <div className="w-full space-y-6">
              {filteredChats.map((chat, i) => (
                <div key={i} className="animate-in fade-in slide-in-from-bottom-2 duration-500 w-full max-w-4xl mx-auto">
                  ...
                  <div className="flex justify-start">
                    <div className="max-w-[85%] bg-purple-900/5 border border-purple-500/10 p-5 rounded-2xl rounded-tl-none shadow-lg">
              ...
              {isProcessing && <div className="flex justify-start animate-pulse max-w-4xl mx-auto w-full px-4"><div className="bg-white/5 border border-white/10 px-4 py-2 rounded-2xl flex items-center gap-3"><div className="flex gap-1.5"><div className="w-1.5 h-1.5 bg-purple-500 rounded-full animate-bounce"></div><div className="w-1.5 h-1.5 bg-purple-500 rounded-full animate-bounce [animation-delay:-0.15s]"></div><div className="w-1.5 h-1.5 bg-purple-500 rounded-full animate-bounce [animation-delay:-0.3s]"></div></div><span className="text-[9px] font-mono text-purple-400 uppercase tracking-widest">{t.injectingMemory}</span></div></div>}
            </div>
          </div>
        </div>
        ...
        <div className="p-4 md:p-6 bg-gradient-to-t from-black via-black/80 to-transparent">
          <div className="max-w-4xl mx-auto flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2 relative">
              <div className="relative">
                {isActionsOpen && (
                  <div className="absolute bottom-full left-0 mb-3 flex flex-col gap-2 animate-in slide-in-from-bottom-2 duration-300">
                    <button onClick={handleLocationQuery} title={t.findLocations} className="w-10 h-10 bg-gray-900 border border-white/10 text-red-500 rounded-full shadow-xl hover:scale-110 transition-transform"><i className="fa-solid fa-location-dot"></i></button>
              ...
              {/* Botón de Manifestación (Marca de Agua) */}
              <button 
                onClick={handleGenerateImage} 
                disabled={!input.trim() || isProcessing || isGeneratingImage}
                title={t.generateImage}
                className={`absolute right-12 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center transition-all duration-500 ${isGeneratingImage ? 'text-yellow-400 animate-spin' : 'text-purple-400/25 hover:text-purple-400 active:scale-90 disabled:opacity-0'}`}
              >
          ...
          <p className="text-center text-[7px] text-gray-800 mt-3 uppercase tracking-[0.4em] font-mono">SFL.046 | Hybrid Persistent Soul | v17.2</p>
        </div>
      </main>
